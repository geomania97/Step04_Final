...
<div class="container">
				...
				<c:choose>
					...
					<c:otherwise>
						...
						<c:if test="${tmp.num ne tmp.comment_group }">
							<li id="reli${tmp.num }" style="padding-left:50px;">
								...
								<dl>
									<dt>
										...
										<%-- 답글 링크를 눌렀을때 해당댓글의 글번호를 얻어오기 위해 data-num 속성에 댓글의 번호 넣어두기 --%>
										<a data-num="${tmp.num }" href="javascript:" class="reply-link">답글</a>
										<%-- 글 작성자가 로그인된 사용자와 같다면 수정, 삭제 링크를 출력한다. --%>
										<c:if test="${ tmp.writer eq sessonScope.id }">
									...
									<dd>
										<%-- 
											댓글은 textarea 로 입력 받았기 때문에 tab , 공백 , 개행 기호도 존재한다.
											html 에서 pre 요소는 tab, 공백, 개행 기호를 해석해주는 요소이기 때문에
											pre 요소의 innerText 로 댓글의 내용을 출력했다.
											그리고 해당 댓글을 javascript 로 바로 수정할수 있도록 댓글 번호를 조합해서 
											아이디를 부여해 놓았다.
										 --%>
								...
								<%-- 
									...
									javascript 에서 폼을 바로 선택할수 있도록 댓글 번호를 조합해서 아이디를 부여해 놓았다.
									댓글의 댓글은 고유한 댓글 그룹번호가 있으므로 form 전송될때 같이 전송되도록 해 놓았다.
									댓글의 그룹번호는 원글의 댓글 글 번호이다.
								 --%>
							...
							<%-- 
								만일 글 작성자가 로그인한 작성자면 ( 본인이 작성한 댓글이라면 ) 댓글 수정 폼도 미리 준비해 놓는다.
								숨겨 둔다.
								javascript 에서 폼을 바로 선택할수 있도록 댓글 번호를 조합해서 아이디를 부여해 놓았다.
							 --%>
	...
	<%-- 
		추가로 댓글을 ajax 요청할때 띄울 회전하는 아이콘 이미지
